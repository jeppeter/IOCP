...
BOOL IOCPS::Startup()
{
	...
	if ( !m_bServerStarted )
	{
		...
		*/
		...
		if ( bRet )
		{
			msg.Format(_T("Successfully started %i logical Worker(s)."),m_nOfWorkers);
			AppendLog(msg);
		}
										   ...
										   IN DWORD dwCallbackData)
{
	...
	IOCPS* pThis = reinterpret_cast<IOCPS*>((UINT_PTR)dwCallbackData);
	...
	if ( pThis )
	{
		while ( !pThis->m_bShutDown )
		{
			...
			if ( events.lNetworkEvents & FD_ACCEPT)
			{
				...
				if ( events.iErrorCode[FD_ACCEPT_BIT] == 0&&pThis->m_bAcceptConnections&&!pThis->m_bShutDown)
				{
					...
					if (clientSocket == SOCKET_ERROR)
					{
						...
						if (nRet != WSAEWOULDBLOCK)
						{
							...
							//
							CString msg;
							msg.Format(_T("Wsaaccept() error: %s"),pThis->ErrorCode2Text(WSAGetLastError()));
							pThis->AppendLog(msg);
						}
					}else
				...
				{
					TRACE("Unknown network event error %ld\n",WSAGetLastError());
					break;
...
{	
	...
	if(m_NumberOfActiveConnections>=m_iMaxNumConnections)
	{
		...
		CString msg;
		msg.Format(_T("Maximum number of Connection is reached. "));
		AppendLog(msg);
	...
	if(pContext!=NULL)
	{
		...
		*/
		...
		{
			CString msg;
			msg.Format(_T("setsockopt() error: %s"),ErrorCode2Text(WSAGetLastError()));
			AppendLog(msg);
		...
		{	
			...
			{
				CString msg;
				msg.Format(_T("ERROR Could not allocate memory for buffer in Acceptincoming: %s"),ErrorCode2Text(WSAGetLastError()));
				AppendLog(msg);
				DisconnectClient(pContext);
				ReleaseClientContext(pContext);
				return FALSE;
		...
		{
			CString msg;
			msg.Format(_T("ERROR HASHMAP AddClientContext failed.  %s"),ErrorCode2Text(WSAGetLastError()));
			AppendLog(msg);
			return FALSE;
		}
	}else
	{
		CString msg;
		msg.Format(_T("ERROR Could not allocate memory for Context in Acceptincoming: %s"),ErrorCode2Text(WSAGetLastError()));
		AppendLog(msg);
		closesocket(clientSocket);
		return FALSE;
	}
...
BOOL IOCPS::AddClientContext(ClientContext *mp)
{
	...
	{
		CString msg;
		msg.Format(_T("Dublicate Key! Disconnecting incoming client."));
		AppendLog(msg);
		AbortiveClose(mp);
		m_ContextMapLock.Unlock();
		return FALSE;
...
void IOCPS::FreeClientContext()
{
	...
	while (pos != NULL) 
	{
		...
		if(pContext)
		{
			...
			CString msg;
			msg.Format(_T("Removed Client=%s."), GetHostKeyName(iKey));
			AppendLog(msg);
	...
	if(pContext&&!m_bShutDown)
	{
		if(pOverlapBuff==NULL)
		{
			...
			if(pOverlapBuff==NULL)
			{
				...
				CString msg;
				msg.Format(_T("FAILED TO Allocate buffer memory in OnRead %s"),ErrorCode2Text(WSAGetLastError()));
				AppendLog(msg);
				DisconnectClient(pContext);
				ReleaseClientContext(pContext);
				return;
			...
			else
			{
				...
				if ( nRetVal == SOCKET_ERROR && WSAGetLastError() != WSA_IO_PENDING) 
				{
					if(WSAGetLastError()!=WSAENOTSOCK)
					{
						...
						{
							CString msg;
							msg.Format(_T("Disconnect in Onread Possible Socket Error: %s"),ErrorCode2Text(WSAGetLastError()));
							AppendLog(msg);
						}
					}
					ReleaseBuffer(pOverlapBuff);
...
void IOCPS::OnReadCompleted(ClientContext *pContext, DWORD dwIoSize,CIOCPBuffer *pOverlapBuff)
{
	...
	{
		CString msg;
		msg.Format(_T("Disconnection in OnReadCompleted (dwIoSize == 0||pOverlapBuff==NULL) errcode: %s"),ErrorCode2Text(WSAGetLastError()));
		AppendLog(msg);
...
void IOCPS::ProcessPackage(ClientContext *pContext, DWORD dwIoSize, CIOCPBuffer *pOverlapBuff)
{
	...
	if ( pBuffPartialMessage!=NULL )
	{
		...
		if ( nUsedBuffer >= MINIMUMPACKAGESIZE )
		{
			...
			{
				CString msg;
				msg.Format(_T("Package size error > MAXIMUMPACKAGESIZE-MINIMUMPACKAGESIZE: %s"),ErrorCode2Text(WSAGetLastError()));
				AppendLog(msg);
				ReleaseBuffer(pOverlapBuff);
				pOverlapBuff=NULL;
				ReleaseBuffer(pContext->m_pBuffOverlappedPackage);
	...
	do 
	{
		...
		if ( nUsedBuffer >= MINIMUMPACKAGESIZE )
		{
			...
			{			
				//pOverlapBuff->DUMP();
				CString msg;
				msg.Format(_T("Messagesize error: %s"),ErrorCode2Text(WSAGetLastError()));
				AppendLog(msg);
#ifdef SIMPLESECURITY
				AddToBanList(pContext->m_Socket);
#endif
			...
			*/  
			...
			if ( nRetVal == SOCKET_ERROR && WSAGetLastError() != WSA_IO_PENDING ) 
			{
				if(WSAGetLastError()!=WSAENOTSOCK)
				{
					...
					{	
						CString msg;
						msg.Format(_T("Error in OnWrite..: %s"),ErrorCode2Text(WSAGetLastError()));
						AppendLog(msg);
					}
				}
				ReleaseBuffer(pOverlapBuff);
	...
	if(pContext!=NULL)
	{
		...
		if (pOverlapBuff!=NULL)
		{
			if(pOverlapBuff->GetUsed()!=dwIoSize)
			{
				...
				CString msg;
				msg.Format(_T("The whole message was not Sent.: %s"),ErrorCode2Text(WSAGetLastError()));
				ReleaseBuffer(pOverlapBuff);
				AppendLog(msg);
			}else
			{
	...
	while (pos != NULL) 
	{
		...
		if(pContext!=NULL)
		{
			...
			if(pOverlapBuff!=NULL)
			{
				...
				{
					CString msg;
					msg.Format(_T("Buffer Error in  ASendToAll, AddData Failed: %s"),ErrorCode2Text(WSAGetLastError()));
					AppendLog(msg);
					ReleaseBuffer(pOverlapBuff);
					bRet&=FALSE;
					break;	
			...
			{
				CString msg;
				msg.Format(_T("Could not allocate memory ASendToAll: %s"),ErrorCode2Text(WSAGetLastError()));
				AppendLog(msg);
				bRet=FALSE;
				break;
			}
...
CString IOCPS::GetHostKeyName(SOCKET socket)
{
	...
	if (bResult != INVALID_SOCKET)
		szReturn.Format(_T("%d>%s_%d"),socket,inet_ntoa(sockAddr.sin_addr), sockAddr.sin_port);
	else
		szReturn.Empty();
...
CString IOCPS::GetHostAddress(SOCKET socket)
{
	...
	if (bResult != INVALID_SOCKET)
		szReturn.Format(_T("%s"), inet_ntoa(sockAddr.sin_addr));
	else
		szReturn.Empty();
	...
	if(pContext)
	{
		...
		if(pOverlapBuff==NULL)
		{
			...
			if(pOverlapBuff==NULL)
			{
				...
				CString msg;
				msg.Format(_T("FAILED TO Allocate buffer memory in OnZeroByteRead: %s"),ErrorCode2Text(WSAGetLastError()));
				AppendLog(msg);
				DisconnectClient(pContext);
				ReleaseClientContext(pContext);
				return; 
		...
		if ( nRetVal == SOCKET_ERROR && WSAGetLastError() != WSA_IO_PENDING) 
		{
			if(WSAGetLastError()!=WSAENOTSOCK)
			{
				...
				{
					CString msg;
					msg.Format(_T("Disconnect in ZeroByteRead Possible Socket Error: %s"),ErrorCode2Text(WSAGetLastError()));
					AppendLog(msg);
				}
			}
			ReleaseBuffer(pOverlapBuff);
...
BOOL IOCPS::AZeroByteRead(ClientContext *pContext, CIOCPBuffer *pOverlapBuff)
{
	...
	if(pContext->m_Socket!=INVALID_SOCKET )
	{
		...
		{            
			...
			ReleaseClientContext(pContext);
			CString msg;
			msg.Format(_T("Disconnect in AZeroByteRead Possible Socket Error: %s"),ErrorCode2Text(WSAGetLastError()));
			AppendLog(msg);
			return FALSE;
		}
...
CIOCPBuffer *IOCPS::AllocateBuffer(int nType)
{
	...
	if(pBuff==NULL)
	{
		...
		{ 
			...
			CString msg;
			msg.Format(_T("Error new CIOCPBuffer(); failed in AllocateBuffer %s"),ErrorCode2Text(WSAGetLastError()));
			AppendLog(msg);
			return NULL;	
		}	
	}
	...
	if(pBuff!=NULL) 
	{
		...
		{	
			CString msg;
			msg.Format(_T(" Error m_BufferList.AddHead((void*)pBuff) failed in AllocateBuffer: %s"),ErrorCode2Text(WSAGetLastError()));
			AppendLog(msg);
			delete pBuff;
			m_BufferListLock.Unlock();
			return NULL;
...
BOOL IOCPS::ReleaseBuffer(CIOCPBuffer *pBuff)
{
	...
	if(pos==NULL)
	{
		...
		CString msg;
		msg.Format(_T("FATAL ERROR pBuff->GetPosition(); returned NULL in ReleaseBuffer: %s"),ErrorCode2Text(WSAGetLastError()));
		AppendLog(msg);
		m_BufferListLock.Unlock();
		return FALSE;
	}
...
void IOCPS::FreeBuffers()
{
	...
	while (pos != NULL) 
	{
		...
		if(pBuff)
		{
			...
			//CString msg;
			//msg.Format(_T("Removed overlap Buffer (type %i)."),pBuff->GetOperation());
			//AppendLog(msg);
...
BOOL IOCPS::SetupListner()
{
	...
	if ( m_socListen == INVALID_SOCKET )
	{
		...
		CString msg;
		msg.Format(_T("Could not create listen socket: %s"),ErrorCode2Text(WSAGetLastError()));
		AppendLog(msg);
		return FALSE;
	}
	...
	{	
		CString msg;
		msg.Format(_T("WSACreateEvent() error : %s"),ErrorCode2Text(WSAGetLastError()));
		AppendLog(msg);
		closesocket(m_socListen);
		return FALSE;
	}
	...
	if ( nRet == SOCKET_ERROR )
	{
		...
		CString msg;
		msg.Format(_T("WSAAsyncSelect() error: %s"),ErrorCode2Text(WSAGetLastError()));
		AppendLog(msg);
		closesocket(m_socListen);
		return FALSE;
	}
	...
	if ( nRet == SOCKET_ERROR )
	{
		...
		CString msg;
		msg.Format(_T("bind() error : %s"),ErrorCode2Text(WSAGetLastError()));
		AppendLog(msg);
		closesocket(m_socListen);
		return FALSE;
	}
	...
	if ( nRet == SOCKET_ERROR )
	{
		...
		CString msg;
		msg.Format(_T("listen() error: %s"),ErrorCode2Text(WSAGetLastError()));
		AppendLog(msg);
		closesocket(m_socListen);
		return FALSE;
	}
	...
	if ( !m_pListenThread )
	{
		...
		CString msg;
		msg.Format(_T("Error Could not start the ListnerThreadProc: %s"),ErrorCode2Text(WSAGetLastError()));
		AppendLog(msg);
		return FALSE;
	}
	return TRUE; 
...
BOOL IOCPS::CreateCompletionPort()
{
	...
	{	
		CString msg;
		msg.Format(_T("Error Could not start Create Socket in CreateCompletionPort: %s"),ErrorCode2Text(WSAGetLastError()));
		AppendLog(msg);
		return FALSE;
	}
	...
	{	
		CString msg;
		msg.Format(_T("CreateIoCompletionPort Failed: %s"),ErrorCode2Text(WSAGetLastError()));
		AppendLog(msg);
		closesocket( s );
		return FALSE;
	}
...
BOOL IOCPS::SetupIOWorkers()
{
	...
	for(int i=0; i<m_iMaxIOWorkers;i++)
	{
		...
		else
		{
			...
			CString msg;
			msg.Format(_T("Error Couldnot start worker: %s"),ErrorCode2Text(WSAGetLastError()));
			AppendLog(msg);
...
BOOL IOCPS::Connect(const CString &strIPAddr, int nPort)
{
	...
	if (clientSocket == INVALID_SOCKET)
	{
		...
		CString msg;
		msg.Format(_T("Connect()->socket() Could not create  socket: %s"),ErrorCode2Text(WSAGetLastError()));
		AppendLog(msg);
		return FALSE;
	}
	// Clear the SockAddr. 
	...
	InetNtop(AF_INET,addr,(PWSTR)((LPCTSTR)strIPAddr),strIPAddr.GetLength());
#else
	...
	{
		CString msg;
		msg.Format(_T("Connection Error: %s"),ErrorCode2Text(WSAGetLastError()));
		AppendLog(msg);
		closesocket(clientSocket);
		return FALSE;
	}else
...
CIOCPBuffer* IOCPS::GetNextSendBuffer(ClientContext *pContext,CIOCPBuffer *pBuff)
{
	...
	if (pBuff!=NULL)
	{
		...
		{
			CString msg;
			msg.Format(_T("Dublicate Key! in SendbufferMap"));
			AppendLog(msg);
			pContext->m_ContextLock.Unlock();
			return NULL;
		}
...
CIOCPBuffer * IOCPS::GetNextReadBuffer(ClientContext *pContext, CIOCPBuffer *pBuff)
{
	...
	if (pBuff!=NULL)
	{
		...
		{
			CString msg;
			msg.Format(_T("Dublicate Key! in ReadbufferMap"));
			AppendLog(msg);
			pContext->m_ContextLock.Unlock();
			return NULL;
		}
		...
		{
			CString msg;
			msg.Format(_T("ERROR CAN NOT SAVE INTO BUFFER!!!"));
			AppendLog(msg);
			pContext->m_ContextLock.Unlock();
			return NULL;
		}
...
BOOL IOCPS::PrepareSendFile(ClientContext *pContext, LPCTSTR lpszFilename)
{
	...
	{    // Already in send mode
		...
		{
			CString msg;
			msg.Format(_T("Could not allocate memory in PrepareSendFile: %s"),ErrorCode2Text(WSAGetLastError()));
			AppendLog(msg);
			DisconnectClient(pContext);
			return FALSE;
		}
	...
	{	
		...
		if(pOverlapBuff!=NULL)
		{
			...
			if ( nRetVal == SOCKET_ERROR && WSAGetLastError() != WSA_IO_PENDING ) 
			{
				...
				{
					CString msg;
					msg.Format(_T("Error in On TransmitFile..: %s"),ErrorCode2Text(WSAGetLastError()));
					AppendLog(msg);
				}
				ReleaseBuffer(pOverlapBuff);
				//DisconnectClient(pContext);
...
void IOCPS::AddToBanList(SOCKET &Socket)
{
	...
	CString msg="";
	msg.Format(_T("%s banned because of bad messagesize."),sIP);
	AppendLog(msg);
	*/
	//
	//  Get the incoming connection IPadress
	...
	if ( nResult != INVALID_SOCKET)
	{
		...
		CString msg="";
		msg.Format(_T("%s banned because of bad messagesize."),inet_ntoa(sockAddr.sin_addr));
		AppendLog(msg);
...
void IOCPS::ClearBanList()
{
	...
	if(!m_BanIPList.IsEmpty())
	{
		m_BanIPList.RemoveAll();
		CString msg="";
		msg.Format(_T("Banned IP List Cleared."));
		AppendLog(msg);
	}
	m_BanIPLock.Unlock();
}
