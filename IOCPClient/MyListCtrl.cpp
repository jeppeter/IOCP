	...
	if(pDispInfo->item.mask & LVIF_TEXT)// If we whant to show text. 
	{
		...
		if(pItem!=NULL)
		{
			...
			case 1: // Second Column		
				...
				{
					float procent=(float)(100.0*pItem->m_iFileBytes/(float)pItem->m_iMaxFileBytes);
					tmp.Format(_T("%0.2f%% %i/%i> "),procent,pItem->m_iFileBytes,pItem->m_iMaxFileBytes);
				}else
					tmp.Format(_T("Number of received packages: %i."),pItem->m_iNumberOfReceivedMsg);
...
void MyListCtrl::OnColumnclick(NMHDR* pNMHDR, LRESULT* pResult) 
{
	...
	if(!pNMListView)
		AfxMessageBox(_T(" C P Fel pNMListView= NULL"));
	else
	{	
...
BOOL MyListCtrl::init()
{
	...
	InsertColumn(0,_T("Client Address"),LVCFMT_LEFT,m_iColumnWidthArray[0]);
	InsertColumn(1,_T("Status"),LVCFMT_LEFT,m_iColumnWidthArray[1]);
