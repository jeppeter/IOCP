...
void CIOCPDlg::OnSend() 
{
	...
	if(m_iCurrentClientID>0)
	{
		...
		for(int i=0;i<20;i++)
		{
			m_sSendTxt.Format(_T("%iCDEFGHIKLMNOPQRST1234567890"),i);
			if(!m_iocp.BuildPackageAndSend(m_iCurrentClientID,m_sSendTxt))
				AfxMessageBox(_T("Error send failed!"));
		}
		if(!m_iocp.BuildPackageAndSend(m_iCurrentClientID,m_sSendTxt))
		{
			DisableClientPanel();
			...
			if(SItem!=-1)
				m_CtrlClientList.SetItemState(SItem,LVNI_ALL, LVIF_TEXT | LVIF_IMAGE | LVIF_STATE);
			m_CtrlClientList.SetFocus();
			AfxMessageBox(_T("Send not successfull.!"));
		}else
		{
			UpdateClientData();
			m_CtrlClientList.SetFocus();
...
void CIOCPDlg::StartTheSystem()
{
	...
	if(!m_iocp.Start(m_ConfigDlg.m_iPortNr,
		...
		AfxMessageBox(_T("Error could not start the Client"));
}
...
void CIOCPDlg::OnBnClickedSendfile()
{
	...
	if( dlg.DoModal()==IDOK )
	{
		...
		{
			AfxMessageBox(_T("Sending transfer info to the client"));
		}else
		{

			m_iocp.DisableSendFile(m_iCurrentClientID);
			AfxMessageBox(_T("Error could not send"));
		}
		
		
	}
#else
	AfxMessageBox(_T("FileTransfer is Disabled. Define TRANSFERFILEFUNCTIONALITY and recompile."));
#endif
}
